#!/usr/bin/env python3
"""
generate_report_md.py
Genera report.md (Markdown) desde SonarQube API.
Uso:
  python3 generate_report_md.py --sonar-url http://192.168.100.236:9000 --project-key test_php --token <TOKEN> --output report.md
"""
import argparse
import requests
...
def get_issues(sonar_url, project_key, token, max_pages=10, page_size=100):
...
def render_markdown(context, template_file, output_file):
    env = Environment(
...
def main():
    ...
    parser.add_argument('--template', default='report_template.md.j2')
    parser.add_argument('--max-issue-pages', default=10, type=int)
    ...
    try:
        ...
        render_markdown(context, args.template, args.output)
